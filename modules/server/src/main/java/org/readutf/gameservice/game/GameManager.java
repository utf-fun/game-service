...
public class GameManager {
    ...
    public GameResult findGame(@NotNull String playlist, List<List<UUID>> teams) {
        ...
        for (Server server : servers) {
            try {
                return requestGame(server, playlist, teams).get(1500, TimeUnit.MILLISECONDS);
            } catch (Exception e) {
                log.error("Failed to request game from server {}: {}", server.getServerId(), e.getMessage());
            }
        }
    ...
    public CompletableFuture<GameResult> requestGame(@NotNull Server server, String playlist, List<List<UUID>> teams) {
